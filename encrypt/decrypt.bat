...
color a
:checkInstance
tasklist | findstr /i "administry.exe" > nul && echo Administry is running... || GOTO ERROR_A06K
:mainmenu
set /p userInputString=<scripts/userInputPending.noid
set savefile=on
::set Decrypt=Nothing
...
set /p ="%DecryptOut%"<nul >scripts/decryptResult.noid
echo.
cls
echo.
echo.
...
:ERROR_A06K
echo ERROR, Administry is not running...
timeout /t 3 >nul
exit